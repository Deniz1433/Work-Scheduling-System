...
const AdminManageUsers = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/roles')
        ...
        .catch(error => {
          ...
          setRoles([]);
          Swal.fire('Hata', 'Roller yüklenirken hata oluştu', 'error');
        });
    ...
    fetch('/api/departments')
        ...
        .catch(error => {
          ...
          setDepartments([]);
          Swal.fire('Hata', 'Departmanlar yüklenirken hata oluştu', 'error');
        });
  }, []);
  ...
  const filteredUsers = (Array.isArray(users) ? users : []).filter(user => {
    const nameMatch = nameFilter === '' || (user.firstName && user.firstName.toLowerCase().includes(nameFilter.toLowerCase()));
    const surnameMatch = filterSurname === '' || (user.lastName && user.lastName.toLowerCase().includes(filterSurname.toLowerCase()));
    const emailMatch = filterEmail === '' || (user.email && user.email.toLowerCase().includes(filterEmail.toLowerCase()));
